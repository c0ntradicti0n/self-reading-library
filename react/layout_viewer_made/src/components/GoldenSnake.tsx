...
const GoldenSnake = ({a = 40, children, ...props}) => {
    ...
    for (let i = 0; i < 11; i++) {
        ...
        if (i>0)
        text_heights.push([x_sum + 500 + 200/(dy/100) *_one, y_sum + (i==2 ? -50:0), x_sum+(dx), y_sum+dy, r_l])
        x_sum += dx
        y_sum += dy
    ...
    path = path.map(p =>p.map(Math.round))
        text_heights = text_heights.map(p =>p.map(p => p*0.3300))
    ...
    return (
        <div ref={ref} style={{ position: 'relative', width: "500px", height:"2000px"}} {...props}>
            <div
                style={{
                    padding: '0px',
                    height: '100vh',
            ...
            >
                <svg
                    ...
                    viewBox={`-250 0 250 1000`}
                >

                    <path
                        rx="10px" ry="10px" stroke-linejoin="round" stroke-linecap="round"
                        d={pathD} fill="none" stroke="black" stroke-width="10"/>
                </svg>
