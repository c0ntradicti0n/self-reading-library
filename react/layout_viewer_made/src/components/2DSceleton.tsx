...
export default function SceletonGraph({
   ...
   width = window.innerWidth * 0.95,
    ...
    const [ForceGraph2D, setFG] = useState( require( 'react-force-graph').ForceGraph2D)
   ...
   useEffect(() => {
      ...
      data.nodes = data.nodes.map((n) => ({ ...n, x: 0, y: 0, value: 100 })).reverse()
      setTrick(!trick)
   }, [data])
    ...
    useEffect(() => {
        if (fgRef && fgRef.current) {
            ...
            fgRef.current.d3Force("link", d3.forceManyBody().strength(-20))
            ...
            <ForceGraph2D
               ...
               nodeCanvasObject={(node, ctx, globalScale) => {
                  ...
                  for (var i = 0; i<lines.length; i++)
                     ctx.fillText(lines[i], node.x,  node.y + ((-lines.length/2 +i)*lineheight) );
