        ...
        with open(model_path + "/.log", "w") as log:
            ...
            for x in range(0, 10):
                ...
                for word, box, label in zip(
                    example["text"][0],
                    model_helpers.compute_bbox(example)[0],
                    example["LABEL"][0],
                ):
                    actual_label = label
                    box = model_helpers.unnormalize_box(box, width, height)
                    draw.rectangle(
                        box, outline=config.label2color[actual_label], width=2
                    )
                    draw.text(
                        (box[0] + 10, box[1] - 10),
                        actual_label,
            ...
            for epoch in range(num_train_epochs):
                print("Epoch:", epoch)
                with tqdm(
                    train_dataloader, total=train_dataloader.dataset.num_rows / 2
                ) as tdqm_train_dataloader:
                ...
                logging.info(
                    f"models in computation: {keeping_models=} with {len(keeping_models)=}"
                )
