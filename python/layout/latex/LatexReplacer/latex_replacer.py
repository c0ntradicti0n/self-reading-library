...
class LatexReplacer(SoupReplacer):
    ...
    def __call__(self, paths, compile=True, *args, **kwargs):
        ...
        for path_to_read_from, meta in paths:
            ...
            with open(".core/log/unreplace.list", "a") as ur:
...
class TestRegexReplacer(unittest.TestCase):
    ...
    def test_normal_math(self):
        ...
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/normal_math.tex") is not None)

    def test_itemize_args(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/item_args.tex") is not None)

    def test_command_open_arg(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/cmd_open_arg.tex") is not None)

    def test_newlines(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/newlines.tex") is not None)

    def test_env(self):
        latex_replacer = LatexReplacer
        assert (latex_replacer.work("latex/LatexReplacer/test/single_feature/env.tex") is not None)

    def test_cmd(self):
        latex_replacer = LatexReplacer
        assert (latex_replacer.work("latex/LatexReplacer/test/single_feature/cmd.tex") is not None)

    def test_input(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/same_dir_input.tex") is not None)

    def test_multicol(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/multicolumn.tex") is not None)

    def test_stycol(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/single_feature/colsty.tex") is not None)

    def test_strange(self):
        latex_replacer = LatexReplacer
        assert (
                latex_replacer.work("latex/LatexReplacer/test/strange/strange.tex") is not None)
