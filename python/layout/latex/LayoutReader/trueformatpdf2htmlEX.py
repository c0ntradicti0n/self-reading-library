...
class PDF_AnnotatorTool(TrueFormatUpmarker):
    ...
    def use_pdf2htmlEX_features(self, path):
    ...
    def determine_layout_label_from_text(self, text):
        return next(
            (label for label in self.label_strings[0:-1] if
             label in text.replace(" ", "")),
    ...
    def generate_css_tagging_document(self, html_read_from="", html_write_to="", parameterizing=False,
        ...
        This has to restore the semantics of the latex, as the reading sequence of left right, top to bottom,
        column for column, page for page. Other latex things should disappear from the text sequence.
        """
        if isinstance(premade_features, pandas.DataFrame) and isinstance(premade_soup, bs4.BeautifulSoup):
            features = premade_features
            soup = premade_soup
        else:
            features, soup = self.compute_html_soup_features(html_read_from)
    ...
    def extract_features(self, html_soup) -> FeatureStuff:
        feature_df = pandas.DataFrame()

        # Collect divs (that they have an x... attribute, that is generated by pdf2htmlEX)
        page_tags = list(self.get_page_tags(html_soup))
        page_to_divs = self.collect_pages_dict(page_tags)

        i = itertools.count()
        feature_df["index"], feature_df["page_number"], feature_df["divs"] = \
            list(list(x) for x in zip(*[(next(i), pn, div) for pn, divs in page_to_divs for div in divs]))
        feature_df["len"] = feature_df.divs.apply(lambda div: len(div.text))

        def get_word_num(x):
            return int("0x" + x.select('z')[0].attrs['class'][0][1:], 0)

        feature_df["word_num"] = feature_df.divs.apply(get_word_num )
        ...
        feature_df = feature_df.drop(columns=['divs'])
        return feature_df

    div_selector = 'div[class*=x]'

    def collect_pages_dict(self, pages):
        page_to_divs = [(page_number, page.select(self.div_selector))
    ...
    def get_declaration_value(self, declaration, key):
        try:
            ...
            self.logger.warning(f"{key} not in css declaration {str(declaration)}")
            return 0
    ...
    def set_css_attributes(self, features):
        ...
        def get_css_attribute(class_and_attribute):
            return self.css_dict_lookup(class_and_attribute)

        for feature_name in concrete_features.columns:
            try:
                features[feature_name] = concrete_features[feature_name].apply(get_css_attribute)
            except KeyError as e:
                self.logger.error(f"Failed to find css class for feature '{feature_name}' " + str(e))
    ...
    def css_dict_lookup(self, css_class_and_attribute):
        try:
            if css_class_and_attribute[0] == None:
                return -1
            if not css_class_and_attribute[0] in self.css_dict:
                self.logger.warning("css selector could not be found in css styles")
                return -1
            return self.get_declaration_value(
                self.css_dict[css_class_and_attribute[0]], css_class_and_attribute[1]
            )
        except Exception as e:
            raise
