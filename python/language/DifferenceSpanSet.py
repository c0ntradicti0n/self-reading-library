...
class DifferenceSpanSet:
    ...
    @cached_property
    def subjects(self):
        return SuperList(reversed(sorted(self._filter_kind(SUBJECT), key=lambda s: s.text)))
    ...
    def side_sets(self):
        return SuperList([
            SuperList([  Span(kind=_kind, word_tags=tag_words)
               for _kind, tag_words in _set
])
            for _set in self.span_sets.values()
            ])
...
class TestSpanMethods(unittest.TestCase):
    ...
    def test_ids(self):
        self.assertEqual(self.dss.subjects.nlp_id,  ['e3ef22cc849bb1c42487ad348e9a8968c5e2bc3704be4b06080dc3f54b880e3c',
 '61ec49b2f01f80373b48baac46459bf964a638374606264bf8701b94884df7f6'] )

    def test_subj_hash(self):
        self.assertEqual(self.dss.subject_hash, "530147de47c462a1ed178ac02794db62f1849d3328b05276a9944c28cc5bde72")

    def test_subj_hash_int(self):
        self.assertEqual(self.dss.subject_hash_int, 605655766414173305614492906248267245040171940863844408278379738501985147138083118269722709201217)


    def test_lemmas(self):
        self.assertEqual(self.dss.contrasts.lemmas, [['believe',
  'in',
  'an',
  'omnipresent',
  ',',
