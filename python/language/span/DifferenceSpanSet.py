...
class Span:
    ...
    SUBJECTS_URI = f"{SUBJECT}"
    CONTRASTS_URI = f"{CONTRAST}"

    FTS_TEXT = f"text"
    FTS_LEMMA = f"lemma"
    WORD_TAGS = f"word_tags"

    FTS_Uris = [FTS_TEXT, FTS_LEMMA]

    def add_graph_literal(self, conn, triples=None):
        Text = self.FTS_TEXT
        Lemma = self.FTS_LEMMA
        WordTags = self.WORD_TAGS

        k = self.nlp_id
        t = self.text
        l = json.dumps(self.lemmas)
        wt = json.dumps(self.word_tags)

        triples_ = [(k, Text, t), (k, Lemma, l), (k, WordTags, wt)]

        conn.add_triples(triples_)
        return k
    ...
    def add_link(self, kind, other, conn):
        ...
        conn.add_triple((a, Kind, b), uri_target=True)

        return [a, b]


class DifferenceSpanSet:
    def __init__(self, d=None, wikidata=None):
        if wikidata:
            self.span_sets = {
                0: [
                    (
                        SUBJECT,
                        Span(SUBJECT, word_tags=wikidata["concept1_label"]).word_tags,
                    ),
                    (
                        CONTRAST,
                        Span(
                            CONTRAST, word_tags=wikidata["concept1_description"]
                        ).word_tags,
                    ),
                ],
                1: [
                    (
                        SUBJECT,
                        Span(SUBJECT, word_tags=wikidata["concept2_label"]).word_tags,
                    ),
                    (
                        CONTRAST,
                        Span(
                            CONTRAST, word_tags=wikidata["concept2_description"]
                        ).word_tags,
                    ),
                ],
            }
    ...
    def init_uris(self, conn):
        if not hasattr(DifferenceSpanSet, "__uris__"):
            ...
            DifferenceSpanSet.d = self.DIFFERENCE
            DifferenceSpanSet.ss = self.SPAN_SETS
    ...
    def add_graph_db(self, conn):
        ...
        sides = unique(sides)
        id = self.hash
        span_sets = json.dumps(self.span_sets)
        for node in sides:
            conn.add_triple((id, DifferenceSpanSet.d, node), uri_target=True)
        conn.add_triple((id, DifferenceSpanSet.ss, span_sets))
...
class TestSpanMethods(unittest.TestCase):
    ...
    def test_wikidata(self):
        v = {
            "contrast1": "http://polarity.science/cb444b8e3c37f39f646a10408e9a54d2",
            "concept1_label": "wiring closet",
            "num": "4",
            "class_label": "room",
            "c1": "cb444b8e3c37f39f646a10408e9a54d2",
            "c2": "5219f9fea7101ecfb1d98488b96b9988",
            "concept1_description": "room used for communications and/or electrical infrastructure",
            "concept2": "http://www.wikidata.org/entity/Q760111",
            "concept1": "http://www.wikidata.org/entity/Q2879748",
            "class": "http://www.wikidata.org/entity/Q180516",
            "contrast2": "http://polarity.science/5219f9fea7101ecfb1d98488b96b9988",
            "concept2_description": "Medical recovery room",
            "concept2_label": "post-anesthesia care unit",
        }
        d = DifferenceSpanSet(wikidata=v)
        self.assertEqual(
            d.subjects.text, ["wiring closet", "post-anesthesia care unit"]
        )
