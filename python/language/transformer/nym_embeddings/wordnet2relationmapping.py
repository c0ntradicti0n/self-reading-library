...
def get_hypernyms(synset):
    ...
    result = set(
        flatten([x if isinstance(x, Synset) else x.syn() for x in result_syns])
    )
    return result
...
def get_pertainyms(synset):
    ...
    for lemma in synset.lemmas():
        ...
        for pertainym in new_pertainyms:
            pertainyms |= set(
                flatten(
                    [list(x.pertainyms()) for x in pertainym.synset().similar_tos()]
                )
            )
    return pertainyms
    ...
    for lemma in synset.lemmas():
        ...
        for derivationally_related_form in derivationally_related_forms:
            derivationally_related_forms |= set(
                flatten(
                    [
                        list(x.derivationally_related_forms())
                        for x in derivationally_related_forms.synset().similar_tos()
                    ]
                )
            )
