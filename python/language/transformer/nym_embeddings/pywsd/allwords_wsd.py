...
from nym_embeddings.pywsd.lesk import simple_lesk, original_lesk
from nym_embeddings.pywsd.similarity import max_similarity
from nym_embeddings.pywsd.stopwords import stopwords
from nym_embeddings.pywsd.tokenize import word_tokenize
from nym_embeddings.pywsd.utils import lemmatize_sentence
...
def disambiguate_tokens(
    ...
    if not context_is_lemmatized:
        surface_words, lemmas, morphy_poss = lemmatize_sentence(
            tokens, keepWordPOS=True, tokenizer=tokenizer
        )
    ...
    for word, lemma, pos in zip(surface_words, lemmas, morphy_poss):
        if lemma not in stopwords:  # Checks if it is a content word
            if wn.synsets(lemma):
                if (
                    algorithm == original_lesk
                ):  # Note: Original doesn't care about lemmas
                    synset = algorithm(tokens, lemma, from_cache=from_cache)
                elif algorithm == max_similarity:
                    synset = algorithm(tokens, lemma, pos=pos, option=similarity_option)
                else:
                    synset = algorithm(
                        tokens,
                        lemma,
    ...
    if prefersNone and not keepLemmas:
        tagged_sentence = [
            (word, None) if str(tag).startswith("#") else (word, tag)
            for word, tag in tagged_sentence
        ]
    if prefersNone and keepLemmas:
        tagged_sentence = [
            (word, lemma, None) if str(tag).startswith("#") else (word, lemma, tag)
...
def disambiguate(
    ...
    if not context_is_lemmatized:
        surface_words, lemmas, morphy_poss = lemmatize_sentence(
            sentence, keepWordPOS=True, tokenizer=tokenizer
        )
        lemma_sentence = " ".join(lemmas)
    else:
        lemma_sentence = (
            sentence  # TODO: Miss out on POS specification, how to resolve?
        )
    for word, lemma, pos in zip(surface_words, lemmas, morphy_poss):
        if lemma not in stopwords:  # Checks if it is a content word
            if wn.synsets(lemma):
                if (
                    ...
                    synset = algorithm(
                        lemma_sentence, lemma, pos=pos, option=similarity_option
                    )
                else:
                    synset = algorithm(
                        lemma_sentence,
