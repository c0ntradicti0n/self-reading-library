...
class ElmoTrain(PathSpec):
    ...
    def __call__(self, feature_meta, *args, **kwargs):
        ...
        try:
            ...
            os.system(
                f"""jq '.train_data_path = "{config.GOLD_SPAN_SET}/train.conll3"' {self.service_config} | sponge {self.service_config}"""
            )
            os.system(
                f"""jq '.validation_data_path = "{config.GOLD_SPAN_SET}/valid.conll3"' {self.service_config} | sponge {self.service_config}"""
            )
