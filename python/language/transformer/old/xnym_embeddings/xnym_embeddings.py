...
def search_in_rowling(M, single_sequence):
    return np.where(
        np.all(np.logical_xor(M == single_sequence, np.isnan(single_sequence)), axis=2)
    )
...
def get_hypernyms(synset):
    ...
    result = set(
        flatten([list(x.lemmas()) if isinstance(x, Synset) else x for x in result_syns])
    )
    return result
...
def get_antonyms(synset):
    ...
    for lemma in synset.lemmas():
        ...
        for antonym in new_antonyms:
            antonyms |= set(
                flatten([list(x.lemmas()) for x in antonym.synset().similar_tos()])
            )
    return antonyms
...
def wordnet_lookup_xnyms(index_to_tokens, fun):
    ...
    for token in lemma_vocab:
        ...
        lemmas = set(
            flatten(
                [list(x.lemmas()) if isinstance(x, Synset) else x for x in xnyms_syns]
            )
        )
...
class XnymEmbedder(TokenEmbedder):
    ...
    def __init__(
        ...
        with timeit_context("creating %s-dict" % self.xnyms):
            ...
            self.xnyms_keys_len_groups = [
                (l, list(g))
                for l, g in itertools.groupby(
                    sorted(self.xnym_dict.items(), key=lambda x: len(x[0])),
    ...
    def generate_xnym_counterparts(self, values):
        ...
        for ac in xnym_counterpars:
            ...
            counterparts = np.array(
                [list(xi) + [np.nan] * (length - len(xi)) for xi in ac]
            )
    ...
    def position_distance_embeddings(self, input_array):
        ...
        for x1_index, s1_indices, p1_index in where_xnyms_match:
            where_counterpart_matches = list(
                search_sequence_numpy(
                    input_array[s1_indices], self.xnyms_counterparts[x1_index]
                )
            )
            ...
            for _, s2_indices, p2_index in where_counterpart_matches:
                ...
                if difference.any():
                    index_sample_token1 = (
                        both_containing_samples,
                        both_containing_positions,
                    )
                    index_sample_token2 = (s1_indices[s2_indices], p2_index)
                    occurrences = np.column_stack(
                        index_sample_token1 + index_sample_token2
                    )
    ...
    def forward(self, inputs: torch.Tensor) -> torch.Tensor:
        ...
        if self.parallelize:
            ...
            with timeit_context("parallel %s-position calculation..." % self.xnyms):
                with Pool(processes=4) as pool:
                    results = pool.map(
                        self.position_distance_embeddings, enumerate(array_slices)
                    )
        ...
        transformer = Normalizer(norm="l2").fit(
            self.S.reshape(-1, self.S.shape[-1])
        )  # fit does nothing.
        tS = transformer.transform(self.S.reshape(-1, self.S.shape[-1])).reshape(
            *self.S.shape
        )
...
class TestingDistanceEmbeddings(unittest.TestCase):
    ...
    def test_embed(self):
        ...
        self.assertEqual(
            cosine_similarity(Es[0].reshape(1, -1), Es[2].reshape(1, -1))[0][0], -1
        )
        self.assertLess(
            cosine_similarity(Es[1].reshape(1, -1), Es[0].reshape(1, -1)), 0
        )
        self.assertEqual(
            cosine_similarity(Es[1].reshape(1, -1), Es[3].reshape(1, -1))[0][0], 0
        )
