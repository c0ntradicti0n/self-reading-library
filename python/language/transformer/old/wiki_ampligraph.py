...
if not os.path.isfile(ke_wnkeys_path) or not os.path.isfile(ke_model_path):
                ...
                for lemma in synset.lemmas():
                    if isinstance(related, Synset):
                        for rel_lemma in related.lemmas():
                            if lemma != rel_lemma:
                                yield list(
                                    (str(lemma.synset()), rel, str(rel_lemma.synset()))
                                )
                    else:
                        if lemma != related:
                            yield list(
                                (str(lemma.synset()), rel, str(related.synset()))
                            )
    ...
    def percentage_split(seq, percentage_dict):
        ...
        return {
            key: seq[a:b]
            for a, b, key in zip([0] + stops, stops, percentage_dict.keys())
        }

    corpus_split_layout = {"train": 0.8, "test": 0.1, "valid": 0.1}
    ...
    X["train"] = np.array(
        [
            list((tok2id[r[0]], r[1], tok2id[r[2]]))
            for r in X["train"]
...
def predict(w1, w2):
    try:
        rellist = [
            [tok2id[str(s1)], k, tok2id[str(s2)]]
            for s1 in wn.synsets(w1)
            for s2 in wn.synsets(w2)
            for k in wordnet_getters.keys()
            if str(s1) in tok2id and str(s2) in tok2id
        ]
        prediction = model.predict(np.array(rellist))
        sorted_predictions = sorted(
            list(zip([p[0] for p in prediction], [r[1] for r in rellist])),
            key=lambda x: x[0],
        )
        print(w1, w2)
...
def plot_clusters(hue):
    ...
    ax = sns.scatterplot(
        data=plot_df[plot_df.continent != "unk"],
        x="embedding1",
        y="embedding2",
        hue=hue,
    )
...
df["results"] = (
    ...
    + (df.home_score < df.away_score).astype(int) * 3
    - 1
)
...
def get_features_target(mask):
    def get_embeddings(team):
        return team_embeddings.get(
            team, np.full(list(team_embeddings.values())[0].shape[0], np.nan)
        )

    X = np.hstack(
        (
            np.vstack(df[mask].home_team_id.apply(get_embeddings).values),
