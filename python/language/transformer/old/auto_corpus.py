...
from server.core.corpus import (
    CONLL_LINE,
    CONLL_LINE_SANITIZE,
    START_LINE,
    ALLOWED_CHARS,
...
class AutoCorpus:
    ...
    def __init__(self, source_corpora=None, target_corpus=None):
        ...
        self.all_annotations = [
            list(zip(instance.fields["tokens"], instance.fields["tags"]))
    ...
    def compare_and_notate(self, document_name, proposals):
        ...
        for proposal in proposals:
            contained_proposals = [
                ...
                if AutoCorpus.contains(proposal, annotation, yet_seen)
            ]
            transposed_proposals = [
                self.transpose_annotation_to_proposal(*cp) for cp in contained_proposals
            ]
    ...
    def contains(proposal, annotation, yet_seen):
        ...
        res = (
            annotation_text in proposal_text
        )  # fuzz.partial_ratio( annotation_text, proposal_text)>0.4
    ...
    def zeroize(sample):
        return "\n".join(
            [
                regex.sub(CONLL_LINE, r"\1  \2  O  O", line)
