...
set_layout = {"train": 0.8, "test": 0.2}


def zeroize(sample):
    return "\n".join(
        [re.sub(conll_line, r"\1  \2  O  O", line) for line in sample.split("\n")]
    )
...
def mix_files():
    for model in models:
        ...
        relevant_files_paths = list(
            get_files_from_recursive_path(path.format(model=model))
        )
        pprint(relevant_files_paths)

        # filtering, changing samples
        all_samples = list(
            flatten([read_conll_file(path) for path in relevant_files_paths])
        )
        ...
        for name, samples in zip(names, tvt):
            ...
            if short_dummy:
                for cp in set_copy[1:]:
                    ...
                    os.system(
                        "cp {path} {cp_path}".format(path=tr_path, cp_path=cp_path)
                    )
