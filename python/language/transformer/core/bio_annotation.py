...
class BIO_Annotation:
    def token_pos_tag_to_conll3(combos):
        ...
        if not count["B"] >= 2:
            logging.error(
                "Annotation contains wrong number of spanning tags!!! %s" % str(count)
            )
            return None
        assert count["B"] >= 2
    ...
    def compute_structured_spans(annotations):
        ...
        nested = [
            ...
            for side in nested
            for sp in side
            if BIO_Annotation.is_nl_text(BIO_Annotation.text_from_span(sp))
        ]
        return nested
    ...
    def spans_from_part(part):
        ...
        partitions = {
            kind: list(tokens)
            for kind, tokens in itertools.groupby(
                part, key=BIO_Annotation.kind_from_tag
            )
        }
    ...
    def annotation_from_spans(
        tokens: List[str], paired_spans: List[List[Dict[str, Any]]]
    ):
        ...
        tags = [
            max(row_tags, key=lambda x: -BIO_Annotation.importance_list.index(x[2:]))
