...
def disambiguate_tokens(tokens, algorithm=simple_lesk,
    ...
    if not context_is_lemmatized:
        surface_words, lemmas, morphy_poss = lemmatize_sentence(tokens, keepWordPOS=True, tokenizer=tokenizer)
        ...
        if lemma not in stopwords: # Checks if it is a content word
            if wn.synsets(lemma):
                if algorithm == original_lesk: # Note: Original doesn't care about lemmas
                    ...
                    synset = algorithm(tokens, lemma, pos=pos, context_is_lemmatized=True,
