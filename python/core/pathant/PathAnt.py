...
class PathAnt:
    def __init__(
        self,
        necessary_paths={config.hidden_folder: ["tex_data", "cache", "log", "topics"]},
    ):
        ...
        for (_froms1, _tos1, functional_object1), (
            ...
            for (_to1, _from1, _to2, _from2) in list_or_values(
                _tos1, _froms1, _tos2, _froms2
            ):
                if _from1 == None:
                ...
                try:
                    if match(_to1, _from2):
                        self.add_edge(
                            _to1,
                            regex.sub(_from2 + "$", _to2, _to1),
                            functional_object2,
                        )
                    if match(_to2, _from1):
                        self.add_edge(
                            _to2,
                            regex.sub(_from1 + "$", _to1, _to2),
                            functional_object1,
                        )
        ...
        try:
            if via:
                if isinstance(via, str):
                    return (
                        self.make_path(G, source=source, target=via)
                        + self.make_path(G, source=via, target=target)[1:]
                    )
                if isinstance(via, list):
                    raise NotImplemented(
                        "Needs to be implemented to have different signs on the way"
                    )
    ...
    def __call__(self, source, target, *args, via=None, filter={}, **kwargs):
        ...
        converters_implications = {
            uv: [_a for _a in a if _a not in converters_path]
        ...
        extra_paths = {
            self.lookup(edge[0], edge[1]): [
                self.estimate_targeting_paths(intermediate_target)
    ...
    def info(self, path="pathant.png", pipelines_to_highlight=None):
        ...
        if pipelines_to_highlight:
            for color, pipeline in enumerate(pipelines_to_highlight):
                pipe_path = self.make_path(
                    dG, pipeline.source, pipeline.target, via=pipeline.via
                )
        ...
        edge_labels = {
            (u, v): f"{a['functional_object']} "
            + (
                "(needs also " + (", ".join(a["implicite"])) + ")"
                if "implicite" in a
                else ""
            )
    ...
    def add_edge(self, froms, tos, functional_object, **kwargs):
        ...
        if isinstance(froms, (List)):
            ...
            for _from in froms:
                others = list(froms)
                others.remove(_from)
                self.add_edge(_from, tos, functional_object, **{"implicite": others})
        ...
        if "*" in _from1:
            ...
            for _from2, _to2 in flatten_optional_list_pair(other_things):
                ...
                if m:
                    ...
                    self.add_starred_from_converters(
                        _to2, new_from, functional_object, converters
                    )

    def add_starred_from_converters(self, _from1, _to1, functional_object, converters):
        other_things = [
            (f, t, functional_object2) for f, t, functional_object2 in converters
        ]
        for _from2, _to2, functional_object2 in flatten_optional_list_triple(
            other_things
        ):
            if "*" in _to2:
