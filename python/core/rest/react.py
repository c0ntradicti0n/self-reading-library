        ...
        if config.create_frontend:
            ...
            if not sync_is_running:
                ...
                echo "while read -r fullpath
                        ...
                        done < <(inotifywait -mr --format '%w%f' -e  modify,create,delete,move {source}/)">sync_patch_app.sh && /bin/bash sync_patch_app.sh &""".format(
                    source=patch_project, target=npm_project
                )
                self.logger.warning(f"Setting up sync")
                os.popen(
                    r"rsync -r {source}/* {target}".format(
                        source=patch_project, target=npm_project
                    )
                )
