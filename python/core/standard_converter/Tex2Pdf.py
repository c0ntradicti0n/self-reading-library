...
class Tex2Pdf(PathSpec):
    ...
    def compiles(self, tex_file_path, n=1, clean=False):
        ...
        ) = get_path_filename_extension(tex_file_path)
        ...
        if clean:
            ...
            subprocess.run(
                ["rm", "*.pdf"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            subprocess.run(
                ["rm", "*.aux"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
            subprocess.run(
                ["rm", "*.log"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL
            )
        ...
        for i in range(n):
            ...
            if any(error in output.lower() for error in ["latex error", "fatal error"]):
                ...
                if line_number_match:
                    ...
                    faulty_code = "\n".join(
                        lines[
                            max(0, line_number - 1) : min(len(lines), line_number + 1)
                        ]
                    )
                    self.path_spec.logger.error(
